...
**https://labqueue-api.onrender.com/docs** — browsable API reference, and you can call
every endpoint from it. No terminal needed.
...
Five clicks to the thing worth seeing:

1. **`POST /auth/login`** → *Test Request* with the credentials above (they are pre-filled
   in the description). Copy the `token` out of the response.
2. **Authorize**, top of the page → paste the token.
3. **`GET /resources`** → copy the `id` of any active instrument.
4. **`POST /reservations`** with `{ resourceId, from, to }` → **201**.
5. **Send it again** → **409**. That is `reservations_no_overlap`, the exclusion constraint
   from Finding A, refusing the second write.

The account is a **member**, not an admin — admin routes can create maintenance windows,
and a public admin login would let any visitor block every resource for everyone after
them. The admin routes are still listed in the reference; they will just return 403.

`GET /` prints the credentials and the same route list as JSON, and the raw document is at
`/openapi/v1.json`. The reference is served in Production, not only in Development — a
docs page that works locally and 404s on the live URL would be the gap it exists to close.
...
tests/LabQueue.Tests xUnit + Testcontainers — 32 tests against real Postgres
```
...
The OpenAPI 3.1 document is generated by `Microsoft.AspNetCore.OpenApi` and rendered by
[Scalar](https://github.com/scalar/scalar) at `/docs`, both served in every environment.
A document transformer adds the JWT bearer scheme and marks the nine operations that
require it, so the reference's *Authorize* control actually signs the requests it sends.
...
| API reference | OpenAPI 3.1 (`Microsoft.AspNetCore.OpenApi`) rendered by Scalar |
...
The API is on `http://localhost:5140`; `/docs` is the browsable reference, `/` prints what
you need to start, and `/health` answers `{"status":"ok"}`. Set `Demo__Seed=true` in `.env` and the API seeds a small demo
